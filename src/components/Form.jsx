			...
			<div className="flex flex-col w-9/12 mx-auto">
				...
				<form onSubmit={handleSubmit} className="flex flex-col bg-gray-900 my-10 border border-gray-600 rounded-lg">
					...
					<label className="m-5">
						...
						<DatePicker selected={startDate} filterDate={(date) => getDisabledDates(date)} minDate={new Date()} onChange={(date) => setStartDate(date)} className="mx-auto text-gray-200 tracking-wider text-lg bg-gray-800 border border-gray-600 hover:bg-gray-700 rounded-lg py-1 px-2 transition duration-300"/>
					</label>

					<label className="m-5">
						<h3 className="text-2xl text-gray-200 tracking-wider mb-2">Description:</h3>
						<textarea value={briefing.desc} onChange={(e) => setBriefing ({...briefing, desc: e.target.value})} placeholder="Lorem ipsum dolor sit amet, consectetur adipiscing elit. Duis non sem fringilla, malesuada nibh sit amet, mollis nunc. Interdum et malesuada fames ac ante ipsum primis in faucibus. Nam varius eu mi in pellentesque. Maecenas a dolor vel enim volutpat." required rows={3} className="flex text-gray-200 tracking-wider text-lg text-justify bg-gray-800 border border-gray-600 hover:bg-gray-700 rounded-lg py-1 px-2 w-full transition duration-300"/>
					</label>
					...
					<label className="m-5">
						...
						<textarea value={briefing.image} onChange={(e) => setBriefing ({...briefing, image: e.target.value})} placeholder="Insert image link here..." rows={2} className="flex text-gray-200 tracking-wider text-lg bg-gray-800 border border-gray-600 hover:bg-gray-700 rounded-lg resize-none py-1 px-2 w-full transition duration-300"/>
					</label>
					...
					<button type='submit' className="flex m-auto my-5 py-1 px-6 text-gray-200 tracking-wider text-2xl bg-red-900 hover:bg-red-400 rounded-lg transition duration-300">
